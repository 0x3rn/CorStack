...
export default function AdminProcessPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/collection/process', {
        method,
        headers: {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = await user.getIdToken();
      const res = await fetch(`/api/admin/collection/process?id=${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-bold text-gray-900">Manage Process</h1>
        {!isEditing && (
      ...
      {isEditing && currentItem ? (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
          <h2 className="text-lg font-bold mb-4">{currentItem.id ? 'Edit Process Step' : 'New Process Step'}</h2>
        ...
        <div className="space-y-4">
          {items.map(item => (
            <div key={item.id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 flex justify-between items-center">
              <div>
                ...
                <span className="text-xs text-gray-400 mt-1 inline-block">Order: {item.order}</span>
              </div>
          ...
          {items.length === 0 && (
            <p className="text-gray-500">No process steps found. Add one above.</p>
          )}
        </div>
      )}
    </div>

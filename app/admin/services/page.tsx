...
export default function AdminServicesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/collection/services', {
        method,
        headers: {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const token = await user.getIdToken();
      const res = await fetch(`/api/admin/collection/services?id=${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
    ...
    <div>
      ...
      {isEditing && currentItem ? (
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
          <h2 className="text-lg font-bold mb-4">{currentItem.id ? 'Edit Service' : 'New Service'}</h2>
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Lucide Icon Name (e.g. Rocket, Layout, Server)</label>
              <input 
                required
                type="text" 
                className="w-full p-2 border rounded"
